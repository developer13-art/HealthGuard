...
import { User, FileText, Phone, MapPin, Building, Calendar, CreditCard, Shield, Hospital, Briefcase } from "lucide-react";
export default function KYCDetailsDialog({ kyc, open, onOpenChange }) {
    if (!kyc)
        return null;
    const InfoRow = ({ icon: Icon, label, value }) => (<div className="flex items-start gap-3 py-2">
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              {kyc.documentCID && (<InfoRow icon={FileText} label="Document CID (IPFS)" value={kyc.documentCID}/>)}
            </CardContent>
          </Card>

          {(kyc.requestedRole || kyc.professionalLicense || kyc.institutionName) && (<Card>
              <CardHeader>
                <CardTitle className="text-sm font-medium flex items-center gap-2">
              ...
              <CardContent className="space-y-2">
                {kyc.requestedRole && (<div className="flex items-start gap-3 py-2">
                ...
                {kyc.professionalLicense && (<InfoRow icon={Shield} label="Professional License" value={kyc.professionalLicense}/>)}
                {kyc.institutionName && (<InfoRow icon={Building} label="Institution Name" value={kyc.institutionName}/>)}
              </CardContent>
            </Card>)}

          {(kyc.selectedHospital || kyc.country || kyc.state || kyc.location || kyc.hospitalProfile) && (<Card>
              <CardHeader>
                <CardTitle className="text-sm font-medium flex items-center gap-2">
              ...
              <CardContent className="space-y-2">
                {kyc.selectedHospital && (<InfoRow icon={Hospital} label="Selected Hospital" value={kyc.selectedHospital}/>)}
                {kyc.country && (<InfoRow icon={MapPin} label="Country" value={kyc.country}/>)}
                {kyc.state && (<InfoRow icon={MapPin} label="State" value={kyc.state}/>)}
                {kyc.location && (<InfoRow icon={MapPin} label="Location" value={kyc.location}/>)}
                {kyc.hospitalProfile && (<div className="flex items-start gap-3 py-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              {kyc.reviewedAt && (<div className="flex items-start gap-3 py-2">
                  <Calendar className="h-4 w-4 text-muted-foreground mt-0.5"/>
                  <div className="flex-1 space-y-1">
              ...
              {kyc.rejectionReason && (<div className="flex items-start gap-3 py-2">
