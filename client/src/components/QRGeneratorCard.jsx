...
export default function QRGeneratorCard() {
    ...
    return (<Card>
      ...
      <CardContent className="space-y-4">
        {!displayQR && !loadingExisting && (<div className="text-center py-8">
            ...
            <Button onClick={() => generateMutation.mutate()} disabled={generateMutation.isPending} data-testid="button-generate-qr">
              {generateMutation.isPending && (<Loader2 className="mr-2 h-4 w-4 animate-spin"/>)}
              Generate QR Code
            </Button>
          </div>)}

        {loadingExisting && (<div className="flex justify-center py-8">
            <Loader2 className="h-8 w-8 animate-spin text-muted-foreground"/>
          </div>)}

        {displayQR && (<div className="space-y-4">
            <Alert>
              <AlertDescription>
            ...
            <div id="qr-code-display" className="flex justify-center p-6 bg-white dark:bg-gray-100 rounded-lg" data-testid="qr-code-display">
              <QRCodeSVG value={displayQR.qrData} size={256} level="H" includeMargin/>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={() => generateMutation.mutate()} variant="outline" className="flex-1" disabled={generateMutation.isPending} data-testid="button-regenerate-qr">
                {generateMutation.isPending ? (<Loader2 className="mr-2 h-4 w-4 animate-spin"/>) : (<RefreshCw className="mr-2 h-4 w-4"/>)}
                Regenerate
              </Button>
            </div>

            {displayQR.scanCount > 0 && (<p className="text-sm text-muted-foreground text-center">
