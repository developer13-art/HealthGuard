...
export default function QRScanner({ onAccessRequested }) {
    ...
    return (<>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {!scanning && !scannedData && (<div className="flex flex-col items-center justify-center py-12 border-2 border-dashed rounded-lg">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full max-w-md">
                    <Button onClick={startScanning} size="lg" className="gap-2 flex-1" data-testid="button-start-scan">
                      <Camera className="h-4 w-4"/>
                      Start Camera
                    </Button>
                    <Button onClick={() => qrFileInputRef.current?.click()} size="lg" variant="outline" className="gap-2 flex-1" disabled={uploadingQR} data-testid="button-upload-qr">
                      {uploadingQR ? (<>
                  ...
                  <input ref={qrFileInputRef} type="file" accept="image/png,image/jpeg,image/jpg" onChange={handleQRFileUpload} className="hidden" data-testid="input-qr-file"/>
              ...
              {scanning && (<div className="space-y-4">
                  <div id="qr-reader" className="w-full rounded-lg overflow-hidden" data-testid="qr-reader"/>
                  <Button onClick={stopScanning} variant="destructive" className="w-full gap-2" data-testid="button-stop-scan">
                    <X className="h-4 w-4"/>
                    Stop Scanning
                  </Button>
                </div>)}

              {cameraError && !scanning && (<Alert variant="destructive">
                  <AlertTriangle className="h-4 w-4"/>
                  <AlertDescription>{cameraError}</AlertDescription>
                </Alert>)}

              {scannedData && !scanning && (<div className="text-center py-8">
                ...
                setScannedData(null);
                setCameraError(null);
            }} className="mt-4" data-testid="button-scan-another">
        ...
        <Card>
          ...
          <CardContent>
            {!scannedData ? (<div className="text-center py-12 text-muted-foreground">
                <User className="h-12 w-12 mx-auto mb-4"/>
                <p>Scan a QR code to view information</p>
              </div>) : (<div className="space-y-4" data-testid="patient-info">
                {scannedData.profilePicture && (<div className="flex justify-center pb-4 border-b">
                ...
                {scannedData.emergencyDetails && (<>
                    ...
                    {scannedData.emergencyDetails.allergies && scannedData.emergencyDetails.allergies.length > 0 && (<div>
                        <p className="text-sm text-muted-foreground">Allergies</p>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {scannedData.emergencyDetails.allergies.map((allergy, i) => (<Badge key={i} variant="destructive">{allergy}</Badge>))}
                        </div>
                      </div>)}
                    {scannedData.emergencyDetails.chronicConditions && scannedData.emergencyDetails.chronicConditions.length > 0 && (<div>
                        <p className="text-sm text-muted-foreground">Chronic Conditions</p>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {scannedData.emergencyDetails.chronicConditions.map((condition, i) => (<Badge key={i} variant="outline">{condition}</Badge>))}
                        </div>
                      </div>)}
                    {scannedData.emergencyDetails.currentMedications && scannedData.emergencyDetails.currentMedications.length > 0 && (<div>
                        <p className="text-sm text-muted-foreground">Current Medications</p>
                        <ul className="list-disc list-inside mt-1 text-sm">
                          {scannedData.emergencyDetails.currentMedications.map((med, i) => (<li key={i}>{med}</li>))}
                        </ul>
                      </div>)}
                    {scannedData.emergencyDetails.emergencyContact && (<div className="border-t pt-4">
                        <p className="text-sm text-muted-foreground">Emergency Contact</p>
                        <p className="font-medium" data-testid="text-emergency-contact">
                          {scannedData.emergencyDetails.emergencyContact}
                        </p>
                        {scannedData.emergencyDetails.emergencyPhone && (<p className="text-sm font-mono" data-testid="text-emergency-phone">
                            {scannedData.emergencyDetails.emergencyPhone}
                          </p>)}
                      </div>)}
                  </>)}
                <div className="pt-4 border-t space-y-2">
                  <Button className="w-full" onClick={() => setShowRequestDialog(true)} data-testid="button-request-details">
        ...
        <DialogContent data-testid="dialog-request-access">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <RadioGroup value={requestType} onValueChange={(v) => setRequestType(v)}>
            ...
            {requestType === "emergency" && (<>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageUpload} className="hidden" data-testid="input-proof-image"/>
                    <Button type="button" variant="outline" onClick={() => fileInputRef.current?.click()} className="w-full" data-testid="button-upload-proof">
                      <FileUp className="h-4 w-4 mr-2"/>
                      {proofImage ? "Change Image" : "Upload Image"}
                    </Button>
                  </div>
                  {proofImage && (<div className="mt-2">
            ...
            <div className="space-y-2">
              ...
              <Textarea id="details" value={proofDetails} onChange={(e) => setProofDetails(e.target.value)} placeholder="Provide details about why you need access..." rows={4} data-testid="textarea-proof-details"/>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowRequestDialog(false)} disabled={submitting} data-testid="button-cancel-request">
              Cancel
            </Button>
            <Button onClick={handleRequestAccess} disabled={submitting} data-testid="button-send-request">
