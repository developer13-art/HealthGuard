...
import { FileCheck, UserCog, Users, Shield, Clock } from "lucide-react";
...
export default function AdminDashboard() {
    ...
    const pendingKYC = kycQueue?.filter((k) => k.status === "pending")?.length || 0;
    const pendingRoles = roleApplications?.filter((r) => r.status === "pending")?.length || 0;
    const totalUsers = users?.length || 0;
    const verifiedUsers = users?.filter((u) => u.status === "verified")?.length || 0;
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {kycQueue && kycQueue.slice(0, 5).map((kyc) => (<div key={kyc.id} className="flex items-center justify-between p-3 rounded-md border">
                <div>
                ...
                <span className={`px-2 py-1 rounded text-xs ${kyc.status === "approved" ? "bg-chart-2/10 text-chart-2" :
                kyc.status === "pending" ? "bg-chart-5/10 text-chart-5" :
                    "bg-destructive/10 text-destructive"}`}>
                  {kyc.status}
                </span>
              </div>))}
            {(!kycQueue || kycQueue.length === 0) && (<p className="text-center text-muted-foreground py-8">No recent activity</p>)}
          </div>
        </CardContent>
      </Card>
    </div>);
