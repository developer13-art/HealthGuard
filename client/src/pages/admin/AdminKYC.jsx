...
export default function AdminKYC() {
    ...
    const reviewedKYC = kycQueue?.filter((k) => k.status !== "pending") || [];
    return (<div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-chart-2" data-testid="count-approved-kyc">
              {reviewedKYC.filter((k) => k.status === "approved").length}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-destructive" data-testid="count-rejected-kyc">
              {reviewedKYC.filter((k) => k.status === "rejected").length}
            </div>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (<div className="text-center py-8 text-muted-foreground" data-testid="loading-kyc">Loading...</div>) : pendingKYC.length > 0 ? (<Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {pendingKYC.map((kyc) => (<TableRow key={kyc.id} data-testid={`row-kyc-${kyc.id}`}>
                    <TableCell className="font-medium" data-testid={`text-kyc-name-${kyc.id}`}>{kyc.fullName}</TableCell>
                    <TableCell>
                      {kyc.requestedRole ? (<Badge variant="secondary" data-testid={`badge-kyc-role-${kyc.id}`}>
                    ...
                    setSelectedKYC(kyc);
                    setDialogOpen(true);
                }} className="gap-1" data-testid={`button-view-kyc-${kyc.id}`}>
                          <Eye className="h-3 w-3"/>
                          View
                        </Button>
                        <Button size="sm" onClick={() => approveMutation.mutate(kyc.id)} disabled={approveMutation.isPending} className="gap-1" data-testid={`button-approve-kyc-${kyc.id}`}>
                          <CheckCircle className="h-3 w-3"/>
                          Approve
                        </Button>
                        <Button size="sm" variant="destructive" onClick={() => rejectMutation.mutate({ kycId: kyc.id, reason: "Invalid documents" })} disabled={rejectMutation.isPending} className="gap-1" data-testid={`button-reject-kyc-${kyc.id}`}>
      ...
      <KYCDetailsDialog kyc={selectedKYC} open={dialogOpen} onOpenChange={setDialogOpen}/>
    </div>);
}
