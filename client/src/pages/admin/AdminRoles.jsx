...
export default function AdminRoles() {
    ...
    return (<div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (<div className="text-center py-8 text-muted-foreground" data-testid="loading-applications">Loading...</div>) : pending.length > 0 ? (<Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                ...
                return (<TableRow key={app.id} data-testid={`row-application-${app.id}`}>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          <Button size="sm" onClick={() => approveMutation.mutate({ userId: app.userId, role: requestedRole })} disabled={approveMutation.isPending} className="gap-1" data-testid={`button-approve-role-${app.id}`}>
                            <CheckCircle className="h-3 w-3"/>
                            Approve
                          </Button>
                          <Button size="sm" variant="destructive" onClick={() => rejectMutation.mutate(app.id)} disabled={rejectMutation.isPending} className="gap-1" data-testid={`button-reject-role-${app.id}`}>
