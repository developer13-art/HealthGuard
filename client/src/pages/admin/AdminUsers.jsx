...
export default function AdminUsers() {
    ...
    return (<div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-chart-2">
              {users?.filter((u) => u.status === "verified").length || 0}
            </div>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (<div className="text-center py-8 text-muted-foreground">Loading...</div>) : (<Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {filteredUsers.map((user) => (<TableRow key={user.id} data-testid={`row-user-${user.id}`}>
                    <TableCell>
                    ...
                    setDialogOpen(true);
                }} className="gap-1" data-testid={`button-view-user-${user.id}`}>
                          <Eye className="h-3 w-3"/>
                          View
                        </Button>
                        {user.status === "verified" ? (<Button size="sm" variant="destructive" onClick={() => toggleStatusMutation.mutate({ userId: user.id, status: "suspended" })} disabled={toggleStatusMutation.isPending || user.role === "admin"} data-testid={`button-suspend-user-${user.id}`}>
                            Suspend
                          </Button>) : user.status === "suspended" ? (<Button size="sm" onClick={() => toggleStatusMutation.mutate({ userId: user.id, status: "verified" })} disabled={toggleStatusMutation.isPending} data-testid={`button-activate-user-${user.id}`}>
                            Activate
                          </Button>) : null}
                      </div>
                    </TableCell>
      ...
      <UserDetailsDialog userId={selectedUserId} open={dialogOpen} onOpenChange={setDialogOpen}/>
    </div>);
}
