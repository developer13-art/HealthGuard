...
export default function DoctorDashboard() {
    ...
    const grantedCount = requests?.filter((r) => r.status === "granted")?.length || 0;
    const assignedPatientsCount = patients?.filter((p) => p.isAssigned)?.length || 0;
    return (<div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {requests && requests.length > 0 ? (<div className="space-y-3">
              {requests.slice(0, 5).map((request) => (<div key={request.id} className="flex items-center justify-between p-3 rounded-md border">
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 rounded text-xs ${request.status === "granted" ? "bg-chart-2/10 text-chart-2" :
            ...
            </div>) : (<p className="text-center text-muted-foreground py-8">No access requests yet</p>)}
        </CardContent>
      </Card>
    </div>);
}
