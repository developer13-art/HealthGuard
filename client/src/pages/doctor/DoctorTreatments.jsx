...
export default function DoctorTreatments() {
    ...
    const thisMonth = treatments?.filter((t) => {
        const date = new Date(t.createdAt);
        const now = new Date();
        return date.getMonth() === now.getMonth() && date.getFullYear() === now.getFullYear();
    }) || [];
    const uniquePatients = new Set(treatments?.map((t) => t.patientId) || []).size;
    return (<div className="space-y-6">
      ...
      <Card>
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <FormField control={form.control} name="hospitalId" render={({ field }) => (<FormItem>
                    ...
                    <FormField control={form.control} name="treatmentDate" render={({ field }) => (<FormItem>
                    ...
                    <FormField control={form.control} name="diagnosis" render={({ field }) => (<FormItem>
                    ...
                    <FormField control={form.control} name="treatment" render={({ field }) => (<FormItem>
                    ...
                    <FormField control={form.control} name="prescription" render={({ field }) => (<FormItem>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        <Input type="file" multiple onChange={handleFileSelect} accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" className="cursor-pointer" data-testid="input-treatment-files"/>
                        <Upload className="h-4 w-4 text-muted-foreground"/>
                      </div>
                      {treatmentFiles.length > 0 && (<div className="space-y-2 mt-2">
                          {treatmentFiles.map((file, index) => (<div key={index} className="flex items-center justify-between bg-muted p-2 rounded-md">
                              <span className="text-sm truncate flex-1">{file.name}</span>
                              <Button type="button" variant="ghost" size="sm" onClick={() => removeFile(index)} data-testid={`button-remove-file-${index}`}>
                                <X className="h-4 w-4"/>
                              </Button>
                            </div>))}
                        </div>)}
        ...
        <CardContent>
          {isLoading ? (<div className="text-center py-8 text-muted-foreground">Loading...</div>) : treatments && treatments.length > 0 ? (<Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {treatments.map((treatment) => (<TableRow key={treatment.id}>
