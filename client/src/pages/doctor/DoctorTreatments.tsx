...
import { z } from "zod";
import { FileCheck, Activity, Users, Plus, Loader2, Upload, X, Paperclip, Eye } from "lucide-react";
...
export default function DoctorTreatments() {
  ...
  const [viewFilesFor, setViewFilesFor] = useState<any | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {/* File viewer dialog */}
          <Dialog open={!!viewFilesFor} onOpenChange={(open) => { if (!open) setViewFilesFor(null); }}>
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                <DialogTitle>Treatment Reference Files</DialogTitle>
                <DialogDescription>
                  Files attached to this treatment log
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {viewFilesFor?.treatmentFiles && viewFilesFor.treatmentFiles.length > 0 ? (
                  viewFilesFor.treatmentFiles.map((file: any, i: number) => (
                    <div key={i} className="flex items-center justify-between border rounded-lg p-3 gap-3">
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <Paperclip className="h-4 w-4 text-muted-foreground flex-shrink-0" />
                        <span className="text-sm truncate">{file.name}</span>
                        <Badge variant="secondary" className="text-xs flex-shrink-0">{file.type?.split("/")[1] || "file"}</Badge>
                      </div>
                      <Button
                        size="sm"
                        variant="outline"
                  ...
                  <p className="text-muted-foreground text-sm text-center py-4">No files attached</p>
                )}
              </div>
            </DialogContent>
          </Dialog>
              ...
              <TableBody>
                {treatments.map((treatment: any) => (
                  <TableRow key={treatment.id}>
                    ...
                    <TableCell>
                      {treatment.treatmentFiles && treatment.treatmentFiles.length > 0 ? (
                        <Button
                          size="sm"
                          variant="outline"
                          className="gap-1 text-xs"
                          onClick={() => setViewFilesFor(treatment)}
                        >
                          <Paperclip className="h-3 w-3" />
                          {treatment.treatmentFiles.length} file{treatment.treatmentFiles.length > 1 ? "s" : ""}
                        </Button>
                      ) : (
