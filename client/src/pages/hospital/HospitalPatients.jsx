...
export default function HospitalPatients() {
    ...
    const requestAccessMutation = useMutation({
        mutationFn: (data) => apiRequest("POST", "/api/hospital/request-access", data),
        onSuccess: () => {
            toast({
    ...
    const handleRequestAccess = () => {
        if (!searchResult || !requestReason.trim())
            return;
    ...
    return (<div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSearch} className="flex gap-2">
            <Input placeholder="Enter UID or username..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} data-testid="input-search-patient"/>
            <Button type="submit" size="sm" disabled={isSearching} data-testid="button-search">
          ...
          {isLoading ? (<div className="flex justify-center py-8">
                ...
                <Table>
                  ...
                  <TableBody>
                    {patients.map((patient) => (<TableRow key={patient.id} data-testid={`row-patient-${patient.id}`}>
                        ...
                        <TableCell>
                          {patient.admissionStatus ? (<Badge variant={patient.admissionStatus === "admitted" ? "default" : "secondary"}>
                              {patient.admissionStatus}
                            </Badge>) : (<span className="text-xs text-muted-foreground">No admission</span>)}
                        </TableCell>
              ...
              <TabsContent value="admitted" className="mt-4">
                <Table>
                  ...
                  <TableBody>
                    {admittedPatients.length === 0 ? (<TableRow>
                        <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                          No currently admitted patients
                        </TableCell>
                      </TableRow>) : (admittedPatients.map((patient) => (<TableRow key={patient.id} data-testid={`row-admitted-${patient.id}`}>
                          <TableCell className="font-mono text-xs">{patient.uid}</TableCell>
                          <TableCell className="font-medium">{patient.username}</TableCell>
                          <TableCell className="text-sm">
              ...
              <TabsContent value="treated" className="mt-4">
                <Table>
                  ...
                  <TableBody>
                    {treatedPatients.length === 0 ? (<TableRow>
                        <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                          No treated patients yet
                        </TableCell>
                      </TableRow>) : (treatedPatients.map((patient) => (<TableRow key={patient.id} data-testid={`row-treated-${patient.id}`}>
                          <TableCell className="font-mono text-xs">{patient.uid}</TableCell>
                          <TableCell className="font-medium">{patient.username}</TableCell>
                          <TableCell className="text-sm">
      ...
      <Dialog open={searchDialogOpen} onOpenChange={setSearchDialogOpen}>
        <DialogContent>
          ...
          {searchResult && (<div className="space-y-4">
              ...
              <Button className="w-full gap-2 mt-4" onClick={() => {
                setSearchDialogOpen(false);
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Textarea id="reason" value={requestReason} onChange={(e) => setRequestReason(e.target.value)} placeholder="e.g., Patient admitted for emergency surgery" rows={4} data-testid="input-access-reason"/>
            </div>
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setRequestDialogOpen(false)}>
                Cancel
              </Button>
              <Button onClick={handleRequestAccess} disabled={!requestReason.trim() || requestAccessMutation.isPending} data-testid="button-submit-request">
