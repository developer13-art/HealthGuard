...
export default function InsuranceConnections() {
    ...
    return (<div className="space-y-6">
      ...
      {/* Pending Requests */}
      {pendingConnections.length > 0 && (<div>
          <h2 className="text-2xl font-bold mb-4">Pending Requests</h2>
          <div className="space-y-4">
            {pendingConnections.map((connection) => (<Card key={connection.id} data-testid={`card-connection-${connection.id}`}>
                <CardHeader>
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <div className="pt-2">
                      <Button onClick={() => handleViewDetails(connection)} className="w-full" data-testid={`button-view-details-${connection.id}`}>
      ...
      {/* Connected Patients */}
      {connectedPatients.length > 0 && (<div>
          <h2 className="text-2xl font-bold mb-4">Connected Patients</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {connectedPatients.map((connection) => (<Card key={connection.id} data-testid={`card-connected-${connection.id}`}>
                <CardHeader>
                ...
                <CardContent>
                  <div className="space-y-2 text-sm">
                    {connection.connectedAt && (<div className="flex justify-between">
                        <span className="text-muted-foreground">Connected Since</span>
                        <span className="font-medium">{new Date(connection.connectedAt).toLocaleDateString()}</span>
                      </div>)}
                    {connection.lastBillingDate && (<div className="flex justify-between">
                    ...
                    {connection.missedPaymentsCount !== undefined && connection.missedPaymentsCount > 0 && (<div className="flex justify-between">
      ...
      {pendingConnections.length === 0 && connectedPatients.length === 0 && (<Card>
      ...
      <Dialog open={detailsDialogOpen} onOpenChange={setDetailsDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto" data-testid="dialog-patient-details">
          ...
          {selectedConnection && (<div className="space-y-6">
              {/* Basic Information */}
              <div>
              ...
              {/* Connection Reason */}
              {selectedConnection.connectionReason && (<div>
              ...
              <div>
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-2">
                    {selectedConnection.allergies && selectedConnection.allergies.length > 0 ? (<div className="flex flex-wrap gap-2">
                        {selectedConnection.allergies.map((allergy, i) => (<Badge key={i} variant="outline" className="bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400">
                            {allergy}
                          </Badge>))}
                      </div>) : (<p className="text-sm text-muted-foreground italic">No known allergies</p>)}
                  </div>
                </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-2">
                    {selectedConnection.chronicConditions && selectedConnection.chronicConditions.length > 0 ? (<div className="flex flex-wrap gap-2">
                        {selectedConnection.chronicConditions.map((condition, i) => (<Badge key={i} variant="outline" className="bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400">
                            {condition}
                          </Badge>))}
                      </div>) : (<p className="text-sm text-muted-foreground italic">No chronic conditions reported</p>)}
                  </div>
                </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-2">
                    {selectedConnection.currentMedications && selectedConnection.currentMedications.length > 0 ? (<div className="flex flex-wrap gap-2">
                        {selectedConnection.currentMedications.map((medication, i) => (<Badge key={i} variant="outline" className="bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400">
                            {medication}
                          </Badge>))}
                      </div>) : (<p className="text-sm text-muted-foreground italic">No current medications</p>)}
                  </div>
                </div>
              </div>

              {/* Emergency Contact */}
              {(selectedConnection.emergencyContact || selectedConnection.emergencyPhone) && (<div>
                  <h3 className="font-semibold text-lg mb-3">Emergency Contact</h3>
                  <div className="grid grid-cols-2 gap-4">
                    {selectedConnection.emergencyContact && (<div>
                        <Label className="text-sm text-muted-foreground">Contact Name</Label>
                        <p className="font-medium">{selectedConnection.emergencyContact}</p>
                      </div>)}
                    {selectedConnection.emergencyPhone && (<div>
          ...
          <DialogFooter className="flex gap-2 sm:gap-2">
            ...
            <Button variant="destructive" onClick={handleRejectFromDialog} disabled={rejectMutation.isPending} data-testid="button-reject-from-details">
              <XCircle className="h-4 w-4 mr-2"/>
              Reject Request
            </Button>
            <Button onClick={handleApproveFromDialog} disabled={approveMutation.isPending} data-testid="button-approve-from-details">
      ...
      <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <DialogContent data-testid="dialog-reject-reason">
          ...
          <div className="space-y-2">
            ...
            <Textarea id="reason" placeholder="e.g., Pre-existing conditions not covered, incomplete medical history..." value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} rows={4} data-testid="textarea-reject-reason"/>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => {
            setRejectDialogOpen(false);
            ...
            <Button variant="destructive" onClick={handleConfirmReject} disabled={rejectMutation.isPending || !rejectReason.trim()} data-testid="button-confirm-reject">
