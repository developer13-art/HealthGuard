...
export default function InsurancePolicies() {
    ...
    const createPolicyMutation = useMutation({
        mutationFn: (data) => apiRequest("POST", "/api/insurance/policies", data),
        onSuccess: () => {
      ...
      <Card>
              ...
              <DialogContent>
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    <FormField control={form.control} name="policyNumber" render={({ field }) => (<FormItem>
                    ...
                    <FormField control={form.control} name="policyName" render={({ field }) => (<FormItem>
                    ...
                    <FormField control={form.control} name="coverageLimit" render={({ field }) => (<FormItem>
                    ...
                    <FormField control={form.control} name="coverageTypes" render={({ field }) => (<FormItem>
                          <FormLabel>Coverage Types (comma-separated)</FormLabel>
                          <FormControl>
                            <Input placeholder="emergency, outpatient, inpatient" {...field} data-testid="input-coverage-types"/>
                          </FormControl>
                          <FormMessage />
                        </FormItem>)}/>
        ...
        <CardContent>
          {isLoading ? (<div className="flex justify-center py-8">
            ...
            </div>) : !policies || policies.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
              ...
              <TableBody>
                {policies.map((policy) => (<TableRow key={policy.id} data-testid={`row-policy-${policy.id}`}>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {policy.coverageTypes?.slice(0, 2).map((type, i) => (<Badge key={i} variant="outline" className="text-xs">
                            {type}
                          </Badge>))}
                        {policy.coverageTypes && policy.coverageTypes.length > 2 && (<Badge variant="outline" className="text-xs">
