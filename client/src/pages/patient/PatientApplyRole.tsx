...
import { uint8ArrayToBase64, uint8ArrayToBase64Url } from "@/lib/encoding";

const ADMIN_WALLET = "HN7cABqLq46Es1jh92dQQisAq662SmxELLLsHHe4YWrH";
  ...
  const handlePayment = async () => {
    ...
    if (!phantomProvider?.isPhantom) {
      toast({
        ...
        description: "Please install Phantom wallet to make payments",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const signerAddress = resp.publicKey.toString();

      const paymentMessage = `HealthGuardX Payment Authorization\n\nRole: ${selectedRole}\nAmount: ${SUBSCRIPTION_AMOUNT} USD\nWallet: ${signerAddress}\nTimestamp: ${Date.now()}`;

      toast({
        title: "Confirm Payment",
        description: "Please confirm the payment authorization in Phantom",
      });

      const encodedMessage = new TextEncoder().encode(paymentMessage);
      const { signature } = await phantomProvider.signMessage(encodedMessage, "utf8");
      const sigBase64 = uint8ArrayToBase64(signature);

      console.log("Payment authorized with signature:", sigBase64);

      const paymentReference = `sig_${uint8ArrayToBase64Url(signature)}`;
      ...
      setPaymentHash(paymentReference);
      ...
      if (error.code === 4001 || error.message?.includes("User rejected")) {
        errorMessage = "You rejected the payment authorization in Phantom. Please try again.";
          ...
          <CardContent className="space-y-4">
            <Alert>
              ...
              <AlertDescription className="space-y-2">
                <p>
                  ...
                  This payment must be authorized via Phantom wallet before you can submit your KYC application.
                </p>
                <p className="text-xs mt-2 pt-2 border-t">
                  <strong>Network Requirements:</strong> This platform runs on the Solana ecosystem. 
                  Make sure you have Phantom wallet installed and your wallet connected to Solana.
                </p>
              </AlertDescription>
            </Alert>
            ...
            <div className="bg-muted/50 p-6 rounded-lg space-y-3">
              ...
              <div className="flex justify-between items-center text-sm text-muted-foreground">
                ...
                <span>Phantom Wallet (Solana)</span>
              </div>
              <div className="flex justify-between items-center text-sm text-muted-foreground">
                <span>Recipient:</span>
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {payingSubscription ? "Processing Payment..." : "Pay 20 USD with Phantom"}
              </Button>
            </div>
          </CardContent>
        </Card>
              ...
              {(selectedRole === "doctor" || selectedRole === "emergency_responder") && (
                <>
                  ...
                  {(selectedRole === "doctor" || selectedRole === "emergency_responder") && (
                    <>
                      <div className="space-y-2">
                        <Label htmlFor="affiliatedHospital">
                          {selectedRole === "emergency_responder" ? "Select Affiliated Hospital (Required)" : "Select Hospital (Required)"}
                        </Label>
                        <Select value={applicationData.affiliatedHospital} onValueChange={(v) => setApplicationData({ ...applicationData, affiliatedHospital: v })}>
