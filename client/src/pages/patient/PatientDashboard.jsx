...
export default function PatientDashboard() {
    ...
    const activeClaimsCount = claims?.filter((c) => c.status === "pending" || c.status === "under_review")?.length || 0;
    ...
    return (<div className="space-y-6">
      ...
      {status !== "verified" && (<Card className="border-destructive">
          <CardHeader>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {quickActions.map((action, index) => (<Card key={index} className="hover-elevate cursor-pointer" onClick={action.onClick}>
              <CardHeader>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {pendingRequests?.slice(0, 3).map((request) => (<div key={request.id} className="flex items-center gap-3 text-sm">
              ...
              {(!pendingRequests || pendingRequests.length === 0) && (<p className="text-center text-muted-foreground py-4">No recent activity</p>)}
            </div>
          </CardContent>
        </Card>
      </div>
