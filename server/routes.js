...
import {
    insertClaimSchema,
    insertPatientInsuranceConnectionSchema,
} from "@shared/schema";
import { ipfsService } from "./ipfs";
// Admin wallet address
const ADMIN_WALLET_ADDRESS =
    "0x82e160da7180AF3e2BB89D7095296d61e139a5a4".toLowerCase();
...
export async function registerRoutes(app) {
    ...
    app.post("/api/auth/connect", async (req, res) => {
        try {
            ...
            if (!user) {
                ...
                const isAdmin =
                    normalizedWalletAddress === ADMIN_WALLET_ADDRESS;
        ...
        try {
            ...
            if (!profilePicture || typeof profilePicture !== "string") {
                return res.status(400).json({
                    error: "Profile picture is required and must be a string (base64 or URL)",
                });
            }
            ...
            if (profilePicture.length > 10 * 1024 * 1024) {
                return res
                    .status(400)
                    .json({ error: "Profile picture size exceeds 10MB limit" });
            }
    ...
    app.post("/api/patient/records", async (req, res) => {
        try {
            ...
            if (req.body.fileData) {
                const fileName =
                    req.body.fileName || `medical_record_${Date.now()}`;
    ...
    app.post("/api/user/qr", async (req, res) => {
        try {
            ...
            const qrData = JSON.stringify({
                ...
                role: user.role,
                hospitalName:
                    user.hospitalName || kycData?.institutionName || null,
    ...
    app.post("/api/patient/access-requests/:id/approve", async (req, res) => {
        try {
            const { id } = req.params;
            console.log(
                `[Approve Access] Approving access request ${id}, setting status to 'granted'`,
            );
            ...
            console.log(
                `[Approve Access] Successfully approved access request ${id}`,
            );
            res.json({ success: true });
        } catch (error) {
    ...
    app.get("/api/patient/insurance", async (req, res) => {
        try {
            ...
            const user = await storage.getUserByWalletAddress(walletAddress);
            if (!user) return res.status(404).json({ error: "User not found" });
            const connections = await storage.getPatientInsuranceConnections(
                user.id,
            );
            res.json(connections);
        } catch (error) {
            res.status(500).json({
                error: "Failed to fetch insurance connections",
            });
        }
    });
    // Get patient claims
    ...
    app.post("/api/patient/subscription-payment", async (req, res) => {
        try {
            ...
            if (!walletAddress) {
                console.error(
                    "[Subscription Payment] No wallet address provided",
                );
                return res
                    .status(400)
            ...
            if (!user) {
                console.error(
                    "[Subscription Payment] User not found for wallet:",
                    walletAddress,
                );
            ...
            if (!transactionHash || !role) {
                console.error(
                    "[Subscription Payment] Missing required fields:",
                    { transactionHash: !!transactionHash, role: !!role },
                );
                return res
                    .status(400)
                    .json({ error: "Transaction hash and role are required" });
            }
            // Check if transaction hash already exists
            const existing =
                await storage.getSubscriptionPaymentByTxHash(transactionHash);
            if (existing) {
                console.error(
                    "[Subscription Payment] Duplicate transaction hash:",
                    transactionHash,
                );
                return res
                    .status(400)
            ...
            if (
                ...
                console.error(
                    "[Subscription Payment] Invalid transaction hash format:",
                    {
            ...
            console.log(
                "[Subscription Payment] Creating payment record:",
                paymentData,
            );
            const payment =
                await storage.createSubscriptionPayment(paymentData);
            console.log(
                "[Subscription Payment] Payment created successfully:",
                payment.id,
            );
            await storage.createAuditLog({
                userId: user.id,
        ...
        try {
            ...
            if (
                ...
                const hasActiveSubscription =
                    await storage.checkActiveSubscription(
                        user.id,
                        requestedRole,
                    );
                if (!hasActiveSubscription) {
                    return res.status(402).json({
                        ...
                        message:
                            "Please complete the 2 BDAG annual subscription payment before submitting your application",
                    });
                }
            }
    ...
    app.post("/api/user/request-access", async (req, res) => {
        try {
            const { patientId, reason, isEmergency, proofImage, proofDetails } =
                req.body;
            const walletAddress = req.headers["x-wallet-address"];
            console.log("[Request Access] Received request:", {
                patientId,
                reason,
                isEmergency,
                walletAddress,
            ...
            const requester =
                await storage.getUserByWalletAddress(walletAddress);
            console.log(
            ...
            const shouldNotifyHospital = !!(
                isEmergency && patient.hospitalName
            );
            console.log("[Request Access] Hospital notification check:", {
                isEmergency,
                hospitalName: patient.hospitalName,
                shouldNotifyHospital,
                type: typeof shouldNotifyHospital,
            });
            const accessData = {
                patientId,
            ...
            await storage.createAuditLog({
                ...
                metadata: {
                    ...
                    reason,
                    isEmergency,
                    hospitalNotified: shouldNotifyHospital,
                },
            });
    ...
    app.get("/api/doctor/search", async (req, res) => {
        try {
            ...
            if (patient.role !== "patient") {
                return res
                    .status(400)
                    .json({ error: "Can only search for patients" });
            }
    ...
    app.get("/api/doctor/treatments", async (req, res) => {
        try {
            ...
            if (!doctor)
                return res.status(404).json({ error: "Doctor not found" });
            const treatments = await storage.getTreatmentLogs(
                undefined,
                doctor.id,
            );
            res.json(treatments);
    ...
    app.post("/api/doctor/treatments", async (req, res) => {
        try {
            ...
            if (doctor.role !== "doctor") {
                return res
                    .status(403)
                    .json({ error: "Only doctors can create treatment logs" });
            }
            ...
            if (!patient) {
                patient = await storage.getUserByUsername(
                    validatedData.patientId,
                );
            }
            if (!patient) {
                return res.status(404).json({
                    error: "Patient not found. Please provide a valid Patient ID, UID, or username.",
                });
            }
            if (patient.role !== "patient") {
                return res.status(400).json({
                    error: "Can only create treatment logs for patients",
                });
            }
            ...
            if (!hasAccess) {
                return res.status(403).json({
                    error: "Access denied. You don't have permission to create treatment logs for this patient.",
                });
            }
            ...
            if (validatedData.hospitalId) {
                ...
                if (!hospital) {
                    hospital = await storage.getUserByUid(
                        validatedData.hospitalId,
                    );
                }
                if (!hospital) {
                    hospital = await storage.getUserByUsername(
                        validatedData.hospitalId,
                    );
                }
                if (hospital) {
                    if (hospital.role !== "hospital") {
                        return res.status(400).json({
                            error: "Invalid hospital ID - must be a hospital user",
                        });
                    }
            ...
            const doctorSignature = CryptoJS.HmacSHA256(
                signatureData,
            ...
            if (hospitalId) {
                const currentAdmission = await storage.getCurrentAdmission(
                    patient.id,
                    hospitalId,
                );
                if (
                    ...
                    await storage.updatePatientAdmissionStatus(
                        currentAdmission.id,
                        "treated",
                    );
                }
    ...
    app.get("/api/doctor/patients", async (req, res) => {
        try {
            ...
            const accessRequests = await storage.getAccessRequestsByRequester(
                doctor.id,
            );
            console.log(
                `[Doctor Patients] Doctor ${doctor.id} has ${accessRequests.length} total access requests`,
            );
            console.log(
                `[Doctor Patients] Access request statuses:`,
                accessRequests.map((req) => req.status),
            );
            const grantedAccess = accessRequests.filter(
                (req) => req.status === "granted",
            );
            console.log(
                `[Doctor Patients] Found ${grantedAccess.length} granted access requests`,
            );
            // Get patients assigned to this doctor via admissions
            const assignedAdmissions = await storage.getPatientAdmissions({});
            const doctorAdmissions = assignedAdmissions.filter(
                (admission) => admission.doctorId === doctor.id,
            );
            ...
            for (const access of grantedAccess) {
                ...
                const admission = doctorAdmissions.find(
                    (a) => a.patientId === access.patientId,
                );
    ...
    app.get("/api/doctor/patient/:patientId/records", async (req, res) => {
        try {
            ...
            if (!hasAccess) {
                return res.status(403).json({
                    error: "Access denied. You don't have permission to view this patient's records.",
                });
            }
    ...
    app.get("/api/doctor/patient/:patientId/profile", async (req, res) => {
        try {
            ...
            if (!hasAccess) {
                return res.status(403).json({
                    error: "Access denied. You don't have permission to view this patient's profile.",
                });
            }
            const patient = await storage.getUser(patientId);
    ...
    app.get("/api/hospital/patients/:patientId/insurance", async (req, res) => {
        try {
            ...
            if (hospital.role !== "hospital") {
                return res
                    .status(403)
                    .json({ error: "Only hospitals can access this endpoint" });
            }
            const patient = await storage.getUserByUid(patientId);
            console.log(
                "[Insurance Lookup] Patient found:",
                patient
                    ? {
                          id: patient.id,
                          uid: patient.uid,
            ...
            console.log(
                "[Insurance Lookup] Found connections:",
                connections.length,
                "connections",
            );
            console.log(
                "[Insurance Lookup] Connection details:",
                connections.map((c) => ({
                    id: c.id,
                    status: c.status,
            ...
            res.status(500).json({
                error: "Failed to fetch patient insurance connections",
            });
        }
    });
    ...
    app.post("/api/hospital/claims", async (req, res) => {
        try {
            ...
            if (hospital.role !== "hospital") {
                return res
                    .status(403)
                    .json({ error: "Only hospitals can submit claims" });
            }
            const claimSchema = insertClaimSchema.omit({
                hospitalId: true,
                treatmentLogId: true,
            });
            ...
            const connection = connections.find(
                (c) => c.id === validatedData.connectionId,
            );
            if (!connection) {
                return res.status(400).json({
                    error: "Invalid insurance connection. The patient is not connected to this insurance provider.",
                });
            }
            if (connection.status !== "connected") {
                return res
                    .status(400)
                    .json({ error: "Insurance connection is not active" });
            }
    ...
    app.post("/api/hospital/treatments", async (req, res) => {
        try {
            ...
            if (hospital.role !== "hospital") {
                return res.status(403).json({
                    error: "Only hospitals can create treatment logs",
                });
            }
            const treatmentSchema = z.object({
                patientId: z.string().min(1, "Patient ID is required"),
            ...
            const hasAccess = await storage.checkAccess(
                patient.id,
                hospital.id,
            );
            if (!hasAccess) {
            ...
            if (validatedData.doctorId) {
                ...
                if (!doctor) {
                    doctor = await storage.getUserByUsername(
                ...
                if (doctor.hospitalName !== hospital.hospitalName) {
                    return res.status(403).json({
                        error: "Doctor must be affiliated with this hospital",
                    });
                }
            }
            const signatureData = `${patient.id}-${validatedData.treatmentDate.toISOString()}-${validatedData.diagnosis}`;
            const signatureKey =
                doctor?.walletAddress || hospital.walletAddress;
            const doctorSignature = CryptoJS.HmacSHA256(
                signatureData,
                signatureKey,
            ).toString();
    ...
    app.get("/api/hospital/treatments", async (req, res) => {
        try {
            ...
            const hospitalTreatments = allTreatments.filter(
    ...
    app.get("/api/hospital/search-patient", async (req, res) => {
        try {
            ...
            const hospitalTreatments = treatments.filter(
                (t) => t.hospitalId === hospital.id,
            );
            // Get patient's claims with this hospital
            const patientClaims = await storage.getClaims({
                patientId: patient.id,
                hospitalId: hospital.id,
            });
            res.json({
                ...
                lastVisit:
                    hospitalTreatments.length > 0
                        ? hospitalTreatments[0].treatmentDate
                        : null,
            });
        } catch (error) {
    ...
    app.get("/api/hospital/patients", async (req, res) => {
        try {
            ...
            const admissions = await storage.getPatientAdmissions({
                hospitalId: hospital.id,
            });
            ...
            const treatedPatientIds = hospitalTreatments.map(
                (t) => t.patientId,
            );
            const allPatientIds = Array.from(
                new Set([...admittedPatientIds, ...treatedPatientIds]),
            );
            ...
            const patients = await Promise.all(
                allPatientIds.map(async (patientId) => {
                    ...
                    const currentAdmission = patientAdmissions.find(
                        (a) =>
                            a.status === "admitted" || a.status === "treated",
                    );
                    const patientTreatments = hospitalTreatments.filter(
                    ...
                    if (currentAdmission?.doctorId) {
                        const doctor = await storage.getUser(
                            currentAdmission.doctorId,
                        );
                    ...
                    return {
                        ...
                        assignedDoctor,
                        isTreated:
                            currentAdmission?.status === "treated" ||
    ...
    app.get("/api/hospital/access-requests", async (req, res) => {
        try {
            ...
            const requests = await storage.getHospitalNotifiedAccessRequests(
    ...
    app.post("/api/hospital/request-access", async (req, res) => {
        try {
            ...
            if (hospital.role !== "hospital") {
                return res
                    .status(403)
                    .json({ error: "Only hospitals can request access" });
            }
            ...
            if (patient.role !== "patient") {
                return res.status(400).json({
                    error: "Can only request access to patient records",
                });
            }
            const accessRequest = await storage.createAccessRequest({
                patientId: patient.id,
    ...
    app.get("/api/hospital/patients-with-access", async (req, res) => {
        try {
            ...
            console.log(
                `[Hospital Patients] Hospital ${hospital.id} has ${accessRequests.length} total access requests`,
            );
            console.log(
                `[Hospital Patients] Access request statuses:`,
                accessRequests.map((req) => req.status),
            );
            const grantedAccess = accessRequests.filter(
                (req) => req.status === "granted",
            ...
            console.log(
                `[Hospital Patients] Found ${grantedAccess.length} granted access requests`,
            );
            // Enrich with patient data
            const patients = await Promise.all(
                grantedAccess.map(async (access) => {
                    const patient = await storage.getUser(access.patientId);
                    const records = await storage.getMedicalRecords(
                        access.patientId,
                    );
                    ...
                    const hospitalClaims = allClaims.filter(
                        (c) => c.hospitalId === hospital.id,
                    );
                    return {
                        id: patient?.id,
                        uid: patient?.uid,
            ...
            console.error(
                "Failed to fetch hospital's patients with access:",
                error,
            );
            res.status(500).json({ error: "Failed to fetch patients" });
        }
    ...
    app.get("/api/emergency/scans", async (req, res) => {
        try {
            ...
            if (responder.role !== "emergency_responder") {
                return res.status(403).json({
                    error: "Access denied. Only emergency responders can access this endpoint.",
                });
            }
    ...
    app.post("/api/emergency/verify-qr", async (req, res) => {
        try {
            ...
            try {
                ...
                return res
                    .status(400)
                    .json({ error: "Invalid QR data format" });
            }
    ...
    app.get("/api/emergency/patients", async (req, res) => {
        try {
            ...
            console.error(
                "Failed to fetch emergency responder's patients:",
                error,
            );
            res.status(500).json({ error: "Failed to fetch patients" });
        }
    ...
    app.get("/api/insurance/claims", async (req, res) => {
        try {
            ...
            const provider = await storage.getInsuranceProviderByUserId(
                user.id,
            );
            if (!provider)
                return res
                    .status(404)
                    .json({ error: "Insurance provider profile not found" });
            // Get all connections for this provider
            const connections = await storage.getProviderConnections(
                provider.id,
            );
            ...
            const allClaims = await Promise.all(
                connectionIds.map((connectionId) =>
                    storage.getClaims({ connectionId }),
                ),
            );
    ...
    app.post("/api/patient/insurance/connect", async (req, res) => {
        try {
            ...
            const connectionSchema =
                insertPatientInsuranceConnectionSchema.omit({
                    patientId: true,
                });
    ...
    app.post("/api/patient/insurance/pay/:connectionId", async (req, res) => {
        try {
            ...
            if (!connection) {
                return res
                    .status(404)
                    .json({ error: "Insurance connection not found" });
            }
            if (connection.status !== "connected") {
                return res
                    .status(400)
    ...
    app.post("/api/insurance/connections/:id/approve", async (req, res) => {
        try {
            ...
            if (!user) {
                console.log(
                    "[Approve] Provider user not found for wallet:",
                    walletAddress,
                );
                return res.status(404).json({ error: "Provider not found" });
            }
            ...
            if (!provider) {
                console.log(
                    "[Approve] Insurance provider profile not found for user:",
                    user.id,
                );
                return res
                    .status(404)
    ...
    app.post("/api/insurance/connections/:id/reject", async (req, res) => {
        try {
            ...
            if (!user) {
                console.log(
                    "[Reject] Provider user not found for wallet:",
                    walletAddress,
                );
                return res.status(404).json({ error: "Provider not found" });
            }
            ...
            if (!provider) {
                console.log(
                    "[Reject] Insurance provider profile not found for user:",
                    user.id,
                );
                return res
                    .status(404)
    ...
    app.get("/api/insurance/analytics", async (req, res) => {
        try {
            ...
            const connectedPatients = connections.filter(
            ...
            const claimTypeAmounts = {
                emergency: claims
                    .filter((c) => c.claimType === "emergency")
                    .reduce((sum, c) => sum + (parseFloat(c.amount) || 0), 0),
                outpatient: claims
                    .filter((c) => c.claimType === "outpatient")
                    .reduce((sum, c) => sum + (parseFloat(c.amount) || 0), 0),
                inpatient: claims
                    .filter((c) => c.claimType === "inpatient")
                    .reduce((sum, c) => sum + (parseFloat(c.amount) || 0), 0),
                surgery: claims
                    .filter((c) => c.claimType === "surgery")
                    .reduce((sum, c) => sum + (parseFloat(c.amount) || 0), 0),
            };
            ...
            res.json({
                ...
                monthlyRevenue:
                    ...
                    parseFloat(provider.monthlyFee || "0"),
            });
        } catch (error) {
    ...
    app.post("/api/insurance/claims/:id/pay", async (req, res) => {
        try {
            ...
            if (claim.status !== "approved") {
                return res
                    .status(400)
                    .json({ error: "Only approved claims can be paid" });
            }
            ...
            for (const connection of connectedPatients) {
                ...
                if (paymentSuccessful) {
                    ...
                    const newMissedCount =
                        (connection.missedPaymentsCount || 0) + 1;
                    await storage.updateConnectionBilling(
                        connection.id,
        ...
        try {
            ...
            if (
                ...
                const providerData = {
                    userId: userId,
                    providerName:
                        kycData.providerName ||
                        kycData.institutionName ||
                        "Insurance Provider",
                ...
                const existingProvider =
                    await storage.getInsuranceProviderByUserId(userId);
                if (!existingProvider) {
    ...
    app.get("/api/consultation/doctors", async (req, res) => {
        try {
            ...
            const doctors = await storage.getUsersByHospital(
                user.hospitalName,
                "doctor",
            );
            res.json(doctors);
    ...
    app.post("/api/consultation/request", async (req, res) => {
        try {
            ...
            if (!patient.hospitalName) {
                return res.status(400).json({
                    error: "You must be assigned to a hospital to request consultation",
                });
            }
            ...
            if (doctor.hospitalName !== patient.hospitalName) {
                return res
                    .status(400)
                    .json({ error: "Doctor must be in the same hospital" });
            }
    ...
    app.get("/api/consultation/:id", async (req, res) => {
        try {
            ...
            if (!consultation) {
                return res
                    .status(404)
                    .json({ error: "Consultation not found" });
            }
    ...
    app.get("/api/chat/:consultationId/messages", async (req, res) => {
        try {
            ...
            if (!user) return res.status(404).json({ error: "User not found" });
            const consultation =
                await storage.getConsultationById(consultationId);
            if (!consultation) {
                return res
                    .status(404)
                    .json({ error: "Consultation not found" });
            ...
            if (consultation.status !== "accepted") {
                return res.status(403).json({
                    error: "Consultation must be accepted to view messages",
                });
            }
    ...
    app.post("/api/chat/:consultationId/messages", async (req, res) => {
        try {
            ...
            if (consultation.status !== "accepted") {
                return res.status(403).json({
                    error: "Consultation must be accepted to send messages",
                });
            }
            const receiverId =
    ...
    app.get("/api/hospital/all-patients", async (req, res) => {
        try {
            ...
            const patients = await storage.getUsersByHospital(
                hospital.hospitalName,
                "patient",
            );
            ...
            const enrichedPatients = await Promise.all(
                patients.map(async (patient) => {
                    ...
                    const latestTreatment = hospitalTreatments.sort(
                        (a, b) =>
                            new Date(b.treatmentDate).getTime() -
                            new Date(a.treatmentDate).getTime(),
                    )[0];
                    ...
                    const hospitalRequests = accessRequests.filter(
                        (r) => r.requesterId === hospital.id,
                    );
                    ...
                    const healthProfile = await storage.getHealthProfile(
                        patient.id,
                    );
                    return {
                        ...patient,
                        ...
                        lastTreatment: latestTreatment
                            ...
                            : null,
                        hasActiveAccess: hospitalRequests.some(
                            (r) => r.status === "granted",
                        ),
        ...
        try {
            ...
            const enrichedDoctors = await Promise.all(
                doctors.map(async (doctor) => {
                    const allTreatments = await storage.getTreatmentLogs();
                    const doctorTreatments = allTreatments.filter(
                        (t) =>
                            t.doctorId === doctor.id &&
                            t.hospitalId === hospital.id,
                    );
                    const latestTreatment = doctorTreatments.sort(
                        (a, b) =>
                            new Date(b.treatmentDate).getTime() -
                            new Date(a.treatmentDate).getTime(),
                    )[0];
                    ...
                    const uniquePatients = new Set(
                        doctorTreatments.map((t) => t.patientId),
                    );
                    ...
                    return {
                        ...
                        specialization:
                            kycData?.professionalLicense || "General Practice",
                        lastActivity:
                            latestTreatment?.treatmentDate || doctor.createdAt,
                    };
                }),
            );
    ...
    app.get("/api/hospital/all-emergency-responders", async (req, res) => {
        try {
            ...
            const responders = await storage.getUsersByHospital(
                hospital.hospitalName,
                "emergency_responder",
            );
            // Enrich with KYC data
            const enrichedResponders = await Promise.all(
                responders.map(async (responder) => {
                    const kycData = await storage.getKYC(responder.id);
                    return {
                        ...responder,
                        professionalLicense:
                            kycData?.professionalLicense || "N/A",
            ...
            res.status(500).json({
                error: "Failed to fetch emergency responders",
            });
        }
    });
    ...
    app.post("/api/hospital/admit-patient", async (req, res) => {
        try {
            ...
            if (!hospital || hospital.role !== "hospital") {
                return res.status(403).json({
                    error: "Forbidden - Only hospitals can admit patients",
                });
            }
            ...
            const { patientId, doctorId, admissionReason, roomNumber, ward } =
            ...
            const existingAdmission = await storage.getCurrentAdmission(
                patientId,
                hospital.id,
            );
            if (existingAdmission) {
                return res.status(400).json({
                    error: "Patient is already admitted to this hospital",
                });
            }
    ...
    app.post("/api/hospital/discharge-patient", async (req, res) => {
        try {
            ...
            if (!hospital || hospital.role !== "hospital") {
                return res.status(403).json({
                    error: "Forbidden - Only hospitals can discharge patients",
                });
            }
            // Validate request body with Zod
            ...
            if (!admission) {
                return res
                    .status(404)
                    .json({ error: "Admission not found for this hospital" });
            }
            // Verify the admission is currently in "admitted" status (not already discharged)
            if (admission.status !== "admitted") {
                return res.status(400).json({
                    error: "Patient is not currently admitted (already discharged)",
                });
            }
            await storage.dischargePatient(
                admissionId,
                hospital.id,
                dischargeNotes,
            );
    ...
    app.post("/api/qr/scan", async (req, res) => {
        try {
            ...
            if (
                scanner.role !== "doctor" &&
                scanner.role !== "emergency_responder" &&
                scanner.role !== "hospital"
            ) {
                return res.status(403).json({
                    error: "Only doctors, emergency responders, and hospitals can scan QR codes",
                });
            }
            if (!qrData) {
            ...
            try {
                ...
                return res
                    .status(400)
                    .json({ error: "Invalid QR code format" });
            }
            // Verify the patient exists
            const patient = await storage.getUserByWalletAddress(
