...
export async function registerRoutes(app) {
    ...
    // AUTH ROUTES
    // ============================================
    // Login endpoint - email/password based
    app.post("/api/auth/login", async (req, res) => {
        try {
            ...
            const allowedRoles = ["patient", "doctor", "hospital", "insurance_provider", "emergency_responder", "admin"];
            ...
            res.json({
                address: user.email || user.walletAddress,
                uid: user.uid,
                role: user.role,
                status: user.status,
            });
    ...
    app.post("/api/doctor/treatments", async (req, res) => {
        try {
            ...
            const doctorSignature = createHmac('sha256', doctor.walletAddress || 'key').update(signatureData).digest('hex');
            const signatureHash = createHash('sha256').update(doctorSignature).digest('hex');
    ...
    app.post("/api/hospital/treatments", async (req, res) => {
        try {
            ...
            const doctorSignature = createHmac('sha256', signatureKey || 'key').update(signatureData).digest('hex');
            const signatureHash = createHash('sha256').update(doctorSignature).digest('hex');
            // Upload treatment record to IPFS
            const treatmentData = {
                patientId: patient.id,
