...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/hospital/all-patients", async (req, res) => {
    try {
      ...
      const allPatients = await storage.getUsersByRole("patient");
      
      // Also get patients directly affiliated with this hospital by name (legacy / explicit affiliation)
      ...
      for (const p of [...allPatients, ...affiliatedByName]) {
  ...
  app.get("/api/hospital/all-doctors", async (req, res) => {
    try {
      ...
      // Build doctor map: affiliated by name + via treatment logs + all system doctors
      ...
      // Fallback: show all doctors in the system if none found through affiliation/treatments
      if (doctorMap.size === 0) {
        const allDoctors = await storage.getUsersByRole("doctor");
        for (const d of allDoctors) doctorMap.set(d.id, d);
      }
      ...
      const enrichedDoctors = await Promise.all(doctors.map(async (doctor) => {
        const doctorTreatments = hospitalTreatments.filter(t => t.doctorId === doctor.id);
  ...
  app.get("/api/hospital/all-emergency-responders", async (req, res) => {
    try {
      ...
      // Get responders affiliated by hospital name
      const affiliatedResponders = hospital.hospitalName
        ? await storage.getUsersByHospital(hospital.hospitalName, "emergency_responder")
        : [];

      // Always fall back to all system emergency responders if none affiliated
      const allResponders = affiliatedResponders.length > 0
        ? affiliatedResponders
        : await storage.getUsersByRole("emergency_responder");
