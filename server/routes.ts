...
// Admin wallet address — Solana public key (case-sensitive base58)
const ADMIN_WALLET_ADDRESS = (process.env.ADMIN_WALLET_ADDRESS || "DXaaCk36zj1DisvdJCbR7wZpYRLReaAYYZx2jYKbneet").trim();
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/connect", async (req, res) => {
    try {
      ...
      if (!user) {
        ...
        // Use first 8 chars of the address as username suffix (works for both Solana & Ethereum)
        ...
        // Elevate existing user to admin if they connect with the admin wallet
  ...
  app.post("/api/patient/access/:id/revoke", async (req, res) => {
    try {
      ...
      // Verify the patient owns this access record
  ...
  // Get all system-wide audit logs (admin only)
  app.get("/api/admin/audit-logs", async (req, res) => {
    try {
      ...
      // Enrich logs with username info
  ...
  // Suspend user for a specific duration or permanently
  app.post("/api/admin/users/:userId/suspend", async (req, res) => {
    try {
      const { userId } = req.params;
      const { durationDays } = req.body; // null or 0 = permanent
      const walletAddress = req.headers["x-wallet-address"] as string;
      const admin = await storage.getUserByWalletAddress(walletAddress);
  ...
  app.get("/api/consultation/doctors", async (req, res) => {
    try {
      ...
      const filtered = specialization
        ? allDoctors.filter((d: any) => d.specialization && d.specialization.toLowerCase().includes(specialization.toLowerCase()))
        : allDoctors;
  ...
  app.post("/api/consultation/request", async (req, res) => {
    try {
      ...
      const videoRoomId = consultationType === "video"
        ? `hgx-${randomBytes(6).toString("hex")}`
        : null;
  ...
  // Patients are not affiliated by hospital_name — find them via admissions, access requests, and treatments
    ...
    try {
      ...
      const [affiliatedByName, affiliatedByKyc, admissions, hospitalTreatments, allPatients] = await Promise.all([
        effectiveHospitalName
          ...
          : Promise.resolve([]),
        effectiveHospitalName
          ? storage.getUsersByKYCAffiliatedHospital(effectiveHospitalName, "patient")
          : Promise.resolve([]),
      ...
      const missingIds = [...new Set([...admittedIds, ...treatedIds])].filter(id => !patientMap.has(id));
      ...
      // Pre-load all KYC and health profiles in parallel (batch, not per-patient)
      ...
      // Build treatment count map from already-loaded hospital treatments
      const treatmentsByPatient = new Map<string, typeof hospitalTreatments>();
      ...
      // Build active admissions map from already-loaded admissions
      const activeAdmissions = new Map<string, typeof admissions[0]>();
      for (const a of admissions) {
        if (a.status === "admitted") activeAdmissions.set(a.patientId, a);
      }

      // Enrich patients using pre-loaded data — no extra DB calls
      const enrichedPatients = patients.map((patient) => {
        const patientTreatments = treatmentsByPatient.get(patient.id) || [];
        const latestTreatment = patientTreatments[0]; // already sorted desc
  ...
  // Shows: affiliated doctors (by hospitalName) + doctors with treatment logs here + all system doctors as fallback
    ...
    try {
      ...
      const [affiliatedDoctors, affiliatedByKyc, hospitalTreatments, allDoctors] = await Promise.all([
        effectiveHospitalName
          ...
          : Promise.resolve([]),
        effectiveHospitalName
          ? storage.getUsersByKYCAffiliatedHospital(effectiveHospitalName, "doctor")
          : Promise.resolve([]),
        // Only fetch treatments for THIS hospital instead of all treatments
        storage.getTreatmentLogs(undefined, undefined, hospital.id),
      ...
      const missingDoctorIds = doctorIdsWithTreatments.filter(id => id && !doctorMap.has(id)) as string[];
      ...
      // Pre-load all KYC data in parallel (batch, not per-doctor)
      const allKyc = await Promise.all(doctors.map(d => storage.getKYC(d.id)));
      const kycMap = new Map(doctors.map((d, i) => [d.id, allKyc[i]]));

      // Build treatment lookup by doctor from already-loaded hospital treatments
      const treatmentsByDoctor = new Map<string, typeof hospitalTreatments>();
      for (const t of hospitalTreatments) {
      ...
      // Enrich using pre-loaded data — no extra DB calls per doctor
      const enrichedDoctors = doctors.map((doctor) => {
        const doctorTreatments = treatmentsByDoctor.get(doctor.id) || [];
        const latestTreatment = doctorTreatments[0]; // already sorted desc
  ...
  // Shows: affiliated responders (by hospitalName) + all system responders as fallback
