...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByWalletAddress(walletAddress: string): Promise<User | undefined> {
    ...
    if (user.status === "suspended" && user.suspendedUntil && new Date() > new Date(user.suspendedUntil)) {
      await db.update(users).set({ status: "verified", suspendedUntil: null }).where(eq(users.id, user.id));
  ...
  async updateUserStatus(id: string, status: string): Promise<void> {
    // When restoring to verified, also clear suspendedUntil
  ...
  async suspendUser(id: string, suspendedUntil: Date | null): Promise<void> {
    await db.update(users).set({ status: "suspended", suspendedUntil }).where(eq(users.id, id));
  }

  async deleteUserPermanently(id: string): Promise<void> {
    // Delete audit logs first (no cascade set on that FK)
  ...
  async getTreatmentLogs(patientId?: string, doctorId?: string, hospitalId?: string): Promise<TreatmentLog[]> {
    const conditions = [];
    if (patientId) conditions.push(eq(treatmentLogs.patientId, patientId));
    if (doctorId) conditions.push(eq(treatmentLogs.doctorId, doctorId));
    if (hospitalId) conditions.push(eq(treatmentLogs.hospitalId, hospitalId));

    if (conditions.length > 0) {
      return await db.select().from(treatmentLogs).where(and(...conditions)).orderBy(desc(treatmentLogs.createdAt));
    }
    return await db.select().from(treatmentLogs).orderBy(desc(treatmentLogs.createdAt));
  }
