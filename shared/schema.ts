...
export const users = pgTable("users", {
  ...
  status: text("status").notNull().default("pending"), // pending, verified, suspended, deleted
  suspendedUntil: timestamp("suspended_until"), // null = permanent suspension, date = temporary
...
export const consultationRequests = pgTable("consultation_requests", {
  ...
  videoRoomId: text("video_room_id"), // Jitsi Meet room ID for video consultations
  paymentPaid: boolean("payment_paid").default(false),
  paymentAmount: integer("payment_amount").default(0), // in cents
  requestedAt: timestamp("requested_at").notNull().defaultNow(),
  respondedAt: timestamp("responded_at"),
  rejectionReason: text("rejection_reason"),
}, (table) => ({
